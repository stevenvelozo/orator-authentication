- Getting Started

  - [Introduction](/)
  - [Quick Start](quickstart.md)
  - [Architecture](architecture.md)

- Core Concepts

  - [Session Management](session-management.md)
  - [Pluggable Authenticator](pluggable-authenticator.md)
  - [Configuration Reference](configuration.md)

- OAuth / OIDC

  - [OAuth Overview](oauth-overview.md)
  - [Google (OIDC)](provider-google.md)
  - [Microsoft (OIDC)](provider-microsoft.md)
  - [Microsoft (MSAL / Exchange)](provider-microsoft-msal.md)
  - [OAuth User Mapper](oauth-user-mapper.md)

- API Reference

  - [Full API Reference](api-reference.md)
  - [setAuthenticator](api-setAuthenticator.md)
  - [setOAuthUserMapper](api-setOAuthUserMapper.md)
  - [setSessionContextResolver](api-setSessionContextResolver.md)
  - [addOAuthProvider](api-addOAuthProvider.md)
  - [connectRoutes](api-connectRoutes.md)
  - [getSessionForRequest](api-getSessionForRequest.md)
